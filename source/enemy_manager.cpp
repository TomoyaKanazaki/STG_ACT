...
void CEnemyManager::Update(void)
{
	...
	for (int nCnt = 0; nCnt < 24; nCnt++)
	{
		if (m_nPopCounter[nCnt] == nTime && m_bPop[nCnt] == false)
		{
			...
			for (int nCntEnemy = 0; nCntEnemy < m_nNumEnemy; nCntEnemy++)
			{
				...
				//敵を生成
				int nType = rand() % CEnemy::TYPE_MAX;
				CEnemy::Create(PopPos, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), (CEnemy::TYPE)nType);
			}
	...
	if (CManager::GetKeyboard()->GetTrigger(DIK_E))
	{
		...
		for (int nCntEnemy = 0; nCntEnemy < m_nNumEnemy; nCntEnemy++)
		{
			...
			//敵を生成
			CEnemy::Create(PopPos, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), CEnemy::TYPE_SHOT);
		}
	}
#endif
}
